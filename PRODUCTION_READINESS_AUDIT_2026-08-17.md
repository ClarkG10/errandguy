...
### 🟡 Medium
`payments.status` unindexed **[FIXED 2026-08-17]** · dispute `resolve()` API is money-blind (Filament path is correct) · `.env.production.example` stale **[FIXED 2026-08-17]** · `WalletTransaction::$display_description` `$appends` lazy-load (latent N+1) **[FIXED 2026-08-17 — accessor no longer lazy-queries; enriches only when booking eager-loaded (the one enriched endpoint already does), field kept in $appends so the JSON contract is unchanged]** · inline FCM sends without timeout **[NON-ISSUE — `config/firebase.php` already sets a 10s `http_client_options.timeout`]** · fragile `tries=1` jobs with no `failed()` **[now covered by the global `Queue::failing` alert]** · unbounded scheduled queries (`CheckRideDurationJob`, message purge) **[FIXED 2026-08-17 — chunkById + batched delete]** · CI deploy fire-and-forget · mobile map/payment keys absent from `eas.json` prod · no log aggregation · OTA no staged rollout · `/health` unmonitored · hard login wall / no guest funnel · OTP resend 5-min · account enumeration on forgot-password (accepted product decision) · profile email/phone change without re-verification **[FIXED 2026-08-17 — changed contact drops its verified flag]**.

### 🟢 Low
`generateBookingNumber()` check-then-insert race **[FIXED 2026-08-17 — suffix widened 4→6 (verified mobile treats booking_number as opaque); unique index backstops the residual]** · file responses skip `nosniff`/`no-store` headers **[FIXED 2026-08-17]** · fat booking-list payload (eager `reviews`) · a few unbounded per-user `->get()` lists · redundant `created_at` index · support IA inconsistency + dead deep-link params · Contacts permission primer before signup · password reset email-only/web-completed · hardcoded cancel reason.
