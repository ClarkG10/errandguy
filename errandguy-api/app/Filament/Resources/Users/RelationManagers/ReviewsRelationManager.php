...
class ReviewsRelationManager extends RelationManager
{
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->paginated([10, 25])
            ->columns([
                TextColumn::make('rating')->badge()->icon('heroicon-m-star')->color(fn ($state): string => (int) $state >= 4 ? 'success' : ((int) $state >= 3 ? 'warning' : 'danger')),
                TextColumn::make('reviewer.full_name')->label('From')->placeholder('—'),
                TextColumn::make('comment')->wrap()->limit(80)->placeholder('—'),
                TextColumn::make('is_flagged')->label('Flagged')->badge()
                    ->formatStateUsing(fn ($state): string => $state ? 'Flagged' : '—')
