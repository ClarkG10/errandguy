...
> **Tech Stack:** React Native (Expo SDK 52+) · TypeScript · NativeWind · Zustand · Expo Router · Laravel 12 · Supabase (PostgreSQL 16) · Mapbox · Stripe/GCash/PayPal
...
- [ ] Create Expo project: `npx create-expo-app ErrandGuy --template blank-typescript`
- [ ] Set Expo SDK version to 52+ in `app.json`
- [ ] Configure `app.json` — app name, slug, scheme (`errandguy`), iOS bundle ID, Android package name, splash screen, icon, adaptive icon, permissions (location, camera, notifications, microphone)
...
- [ ] Create `tsconfig.json` — set strict mode, path aliases (`@/*` → `./src/*`)
- [ ] Create `.eslintrc.js` — Expo + TypeScript rules
- [ ] Create `.prettierrc` — consistent formatting

### 0.2 — Install Mobile Dependencies

- [ ] Core navigation: `expo-router`, `react-native-screens`, `react-native-safe-area-context`, `react-native-gesture-handler`
- [ ] Styling: `nativewind`, `tailwindcss` — create `tailwind.config.js` with custom ErrandGuy theme
...
- [ ] Fonts: `expo-font`, `@expo-google-fonts/montserrat`
- [ ] Haptics: `expo-haptics`
- [ ] Date/time: `dayjs`
- [ ] Forms: `react-hook-form`, `zod` (validation)
- [ ] Keyboard: `react-native-keyboard-aware-scroll-view` 
- [ ] Payments: `@stripe/stripe-react-native`, `react-native-paypal` (or PayPal SDK)
...
- [ ] Create `global.css` — import Tailwind directives
- [ ] Update `babel.config.js` — add NativeWind + Reanimated plugins

### 0.4 — Backend (Laravel) Initialization

- [ ] Create Laravel 12 project: `composer create-project laravel/laravel errandguy-api`
- [ ] Configure `.env` file:
  - `DB_CONNECTION=pgsql`
...
- [ ] Create directory structure under `app/`:
  ...
  └── Notifications/
  ```
- [ ] Create `routes/api.php` — empty route groups for auth, customer, runner, admin, chat, payments, notifications
- [ ] Create API versioning middleware or prefix (`/api/v1/`)
...
- [ ] Create `src/constants/colors.ts` — export light/dark color objects:
...
- [ ] Create `src/constants/typography.ts` — font sizes (xs 12, sm 14, base 16, lg 18, xl 20, 2xl 24, 3xl 30), font families (`Montserrat_400Regular`, `Montserrat_700Bold`), line heights
- [ ] Create `src/constants/spacing.ts` — spacing scale (4, 8, 12, 16, 20, 24, 32, 40, 48, 64)
- [ ] Create `src/constants/radius.ts` — border radius values (sm 6, md 10, lg 14, xl 20, full 9999)
- [ ] Create `src/constants/animations.ts` — spring config (`damping: 15, stiffness: 150`), fade duration (`200ms`), timing config
...
- [ ] Create `src/types/payment.ts`:
  ...
  - `PaymentMethodType` enum: `'card' | 'gcash' | 'paypal' | 'wallet' | 'cash'`
...
- [ ] Create `src/services/api.ts` — Axios instance:
  ...
  - Timeout: 30 seconds
- [ ] Create `src/services/supabase.ts` — Supabase client:
...
- [ ] Create `src/services/runner.service.ts`:
...
- [ ] Create `src/stores/runnerStore.ts`:
...
- [ ] Create `SOSConfirmationModal.tsx`:
...
- [ ] Create `useAuth.ts` — wraps `authStore`, provides `login()`, `logout()`, `isAuthenticated`, `user`, `role`
- [ ] Create `useLocation.ts` — wraps `expo-location`, provides `currentLocation`, `requestPermissions()`, `startWatching()`, `stopWatching()`
- [ ] Create `useNotifications.ts` — wraps `expo-notifications`, provides `registerForPush()`, `handleNotification()`, `expoPushToken`
- [ ] Create `useBooking.ts` — wraps `bookingStore`, provides booking flow helpers, `createBooking()`, `cancelBooking()`, `activeBooking`
- [ ] Create `useChat.ts` — wraps `chatStore` + Supabase Realtime subscription for live messages
- [ ] Create `useRunnerTracking.ts` — Supabase Realtime subscription for `runner_locations` table, returns `runnerLocation`, `isConnected`
- [ ] Create `useDebounce.ts` — generic debounce hook for search inputs
- [ ] Create `useKeyboard.ts` — keyboard visibility and height tracking
- [ ] Create `useImagePicker.ts` — wraps `expo-image-picker`, provides `pickImage()`, `takePhoto()`, returns `uri`, `base64`
- [ ] Create `useCountdown.ts` — countdown timer hook (for incoming request 30s timer, OTP expiry)
- [ ] Create `useRefreshOnFocus.ts` — re-fetch data when screen comes into focus (Expo Router)
- [ ] Create `useSupabaseRealtime.ts` — generic hook to subscribe to any Supabase Realtime channel

### 1.7 — Utility Functions (`src/utils/`)

- [ ] Create `formatCurrency.ts` — format numbers to Philippine Peso (`₱`) with commas
- [ ] Create `formatDate.ts` — date/time formatting helpers using `dayjs` (relative time, full date, time only)
- [ ] Create `formatDistance.ts` — format km to readable string (e.g., "2.5 km", "800 m")
- [ ] Create `validators.ts` — phone number validation (PH format), email validation, password strength validation
- [ ] Create `storage.ts` — wrapper around `expo-secure-store` for token storage, `AsyncStorage` for non-sensitive data
- [ ] Create `permissions.ts` — centralized permission request functions (location, camera, notifications)
- [ ] Create `mapUtils.ts` — coordinate helpers (distance between two points, region from coordinates, polyline decode)
- [ ] Create `imageUtils.ts` — image resize, compression before upload
- [ ] Create `generatePin.ts` — generate random 4-digit PIN for ride verification

### 1.8 — App Constants (`src/constants/`)

- [ ] Create `errandTypes.ts` — list of 7 errand type slugs and display names:
  - `delivery` — "Delivery"
...
- [ ] Create `quickMessages.ts` — pre-canned chat messages for customer and runner:
...
- [ ] Create `statusLabels.ts` — human-readable labels for each `BookingStatus`
- [ ] Create `config.ts` — app-wide config constants (API timeout, max file size, pagination limit, OTP length, countdown duration)

### 1.9 — Root Layout & Navigation Guards

- [ ] Implement `src/app/_layout.tsx`:
  ...
  - Handle deep links
- [ ] Implement `src/app/(auth)/_layout.tsx`:
  - Stack navigator for auth screens (no tab bar)
  - Redirect to main app if already authenticated
- [ ] Implement `src/app/(customer)/_layout.tsx`:
  - Redirect to auth if not authenticated
  - Redirect to runner if role is runner
- [ ] Implement `src/app/(customer)/(tabs)/_layout.tsx`:
...
- [ ] Implement `src/app/(runner)/_layout.tsx`:
  - Redirect to auth if not authenticated
  - Redirect to customer if role is customer
- [ ] Implement `src/app/(runner)/(tabs)/_layout.tsx`:
  - Bottom tab navigator with 4 tabs:
...
- [ ] **Migration 008:** Create `payments` table
  - Columns: `id` (UUID, PK), `booking_id` (UUID, FK → bookings.id), `customer_id` (UUID, FK → users.id), `amount` (decimal 10,2), `currency` (varchar 5, default 'PHP'), `method` (varchar 15, check: 'card'/'gcash'/'paypal'/'wallet'/'cash'), `status` (varchar 15, default 'pending', check: 'pending'/'processing'/'completed'/'failed'/'refunded'), `gateway_tx_id` (varchar 100, nullable), `gateway_response` (jsonb, nullable), `paid_at` (timestamptz, nullable), `refund_amount` (decimal 10,2, nullable), `refunded_at` (timestamptz, nullable), `created_at` (timestamptz), `updated_at` (timestamptz)
...
- [ ] `runner_locations` table RLS:
  - Runner can insert own locations
  - Customer can read locations for their active booking
- [ ] `notifications` table RLS:
  - Users can read own notifications
- [ ] Apply similar scoped RLS for all remaining tables

### 2.3 — Supabase Realtime Configuration

- [ ] Enable Realtime for `messages` table — broadcast inserts to booking channel
- [ ] Enable Realtime for `runner_locations` table — broadcast inserts to tracking channel
- [ ] Enable Realtime for `bookings` table — broadcast updates for status changes
- [ ] Enable Realtime for `notifications` table — broadcast inserts to user channel
- [ ] Define channel naming convention:
...
- [ ] `create_runner_profiles_table` migration
- [ ] `create_runner_documents_table` migration
- [ ] `create_errand_types_table` migration
- [ ] `create_bookings_table` migration
- [ ] `create_booking_status_logs_table` migration
- [ ] `create_runner_locations_table` migration
- [ ] `create_payments_table` migration
- [ ] `create_payment_methods_table` migration
- [ ] `create_wallet_transactions_table` migration
- [ ] `create_messages_table` migration
- [ ] `create_reviews_table` migration
- [ ] `create_notifications_table` migration
- [ ] `create_saved_addresses_table` migration
- [ ] `create_trusted_contacts_table` migration
- [ ] `create_sos_alerts_table` migration
- [ ] `create_promo_codes_table` migration
- [ ] `create_admin_users_table` migration
- [ ] `create_dispute_tickets_table` migration
- [ ] `create_system_config_table` migration
- [ ] `create_personal_access_tokens_table` migration (Sanctum)
...
- [ ] Create `Notification.php` (model, not Laravel notification):
  - Fillable fields
  - Casts: `data` → array, `is_read` → boolean
  - Relationships: `belongsTo(User)`
  - Scopes: `scopeUnread()`

- [ ] Create `SavedAddress.php`:
  - Fillable fields
  - Casts: `is_default` → boolean
  - Relationships: `belongsTo(User)`

- [ ] Create `TrustedContact.php`:
  - Fillable fields
...
- [ ] Create `ErrandTypeSeeder.php` — seed 7 default errand types with base fees and per-km rates
- [ ] Create `SystemConfigSeeder.php` — seed default system config values
- [ ] Create `AdminUserSeeder.php` — seed initial super_admin account
- [ ] Update `DatabaseSeeder.php` — call all seeders in order
...
- [ ] Create `app/Http/Middleware/EnsureUserActive.php`:
...
- [ ] Add API security headers middleware (`app/Http/Middleware/SecurityHeaders.php`):
...
- [ ] Create `PasswordResetController.php`:
...
- [ ] Create `app/Http/Resources/RunnerProfileResource.php`:
...
- [ ] Define auth routes (no middleware, rate limited):
  ```
...
- [ ] Implement splash screen behavior in `src/app/_layout.tsx`:
...
- [ ] Build onboarding slides (3 slides):
...
- [ ] Storage: Set `@onboarding_seen = true` in AsyncStorage so slides only show once
- [ ] Navigation: "Get Started" → navigate to `login`
...
- [ ] Accept params: `phone` or `email`, `purpose` (login-verify / register-verify)
- [ ] Build OTP form:
...
- [ ] Build registration form (scrollable):
...
- [ ] Create `TrustedContactRequest.php`:
...
- [ ] Create `SavedAddressController.php`:
...
- [ ] Create `TrustedContactController.php`:
...
- [ ] Create `CreateBookingRequest.php`:
  - Validated fields:
...
- [ ] Create `app/Services/PromoService.php`:
...
- [ ] Create `app/Http/Resources/ErrandTypeResource.php`:
...
- [ ] Create `app/Policies/TrustedContactPolicy.php`:
...
- [ ] Define user profile routes (middleware: `auth:sanctum`, `active`):
  ```
  ...
  ```
- [ ] Define booking routes (middleware: `auth:sanctum`, `active`, `role:customer`):
  ```
...
- [ ] Create `app/Events/BookingCreated.php` — fired when booking created, triggers runner matching
- [ ] Create `app/Events/BookingStatusChanged.php` — fired on every status change, triggers notifications + Supabase broadcast
- [ ] Create `app/Events/BookingCancelled.php` — fired on cancellation, triggers refund + runner notification
- [ ] Create `app/Jobs/MatchRunnerJob.php` — async job that runs `MatchingService::findRunner()`
- [ ] Create `app/Jobs/BroadcastToRunnersJob.php` — async job for negotiate mode broadcasting
- [ ] Create `app/Jobs/ExpireNegotiateBookingJob.php` — scheduled job, runs at `negotiate_expires_at`, cancels if no runner accepted
- [ ] Create `app/Jobs/AutoCancelBookingJob.php` — scheduled job, cancels booking if no runner found within timeout (from system_config `auto_cancel_timeout_minutes`)
- [ ] Create `app/Listeners/SendBookingNotification.php` — listens to `BookingStatusChanged`, sends push notification to customer/runner
...
- [ ] Components needed in `src/components/customer/`:
  - [ ] Create `ErrandTypeCard.tsx` — icon + label card, selectable state, tap handler
  - [ ] Create `ActiveErrandBanner.tsx` — status chip, runner name, CTA button, animated progress line
  - [ ] Create `PromoBanner.tsx` — background gradient card, promo title, discount text
  - [ ] Create `RecentErrandItem.tsx` — errand type icon, description preview, date, amount, status badge

### 6.2 — Booking Flow Step 1: Errand Type (`src/app/(customer)/book/type.tsx`)

- [ ] Build errand type selection grid:
...
- [ ] Data: use cached errand types from `configService.getErrandTypes()`

### 6.3 — Booking Flow Step 2: Task Details (`src/app/(customer)/book/details.tsx`)

- [ ] Build task details form (scrollable):
...
- [ ] State: save all fields to `bookingStore.draftBooking`
- [ ] Components needed:
  - [ ] Create `src/components/customer/AddressInput.tsx` — text input with Mapbox Places autocomplete suggestions dropdown, "Use Current Location" button, saved addresses shortcut
  - [ ] Create `src/components/customer/PhotoGrid.tsx` — grid of photo thumbnails, add button, remove button overlay
  - [ ] Create `src/components/customer/MiniRouteMap.tsx` — Mapbox MapView showing pickup/dropoff markers + polyline route
  - [ ] Create `src/components/customer/SavedAddressSheet.tsx` — `BottomSheet` listing saved addresses, tap to select

### 6.4 — Booking Flow Step 3: Schedule (`src/app/(customer)/book/schedule.tsx`)

- [ ] Build schedule selection:
  ...
  - "Continue" `Button`
- [ ] State: save `schedule_type` and `scheduled_at` to `bookingStore.draftBooking`
- [ ] Components needed:
  - [ ] Create `src/components/customer/ScheduleToggle.tsx` — "Now" / "Later" toggle cards
  - [ ] Create `src/components/customer/DateTimePicker.tsx` — calendar + time picker combined

### 6.5 — Booking Flow Step 4: Review & Payment (`src/app/(customer)/book/review.tsx`)

- [ ] Build review screen — two modes based on `pricing_mode`:
...
- [ ] Components needed:
  - [ ] Create `src/components/customer/VehicleTypeSelector.tsx` — 4 vehicle cards with icon, ETA, price; selectable
  - [ ] Create `src/components/customer/PromoCodeInput.tsx` — input + Apply button + result chip
  - [ ] Create `src/components/customer/PaymentMethodSelector.tsx` — current method display + BottomSheet picker
  - [ ] Create `src/components/customer/OfferSlider.tsx` — custom slider with amount display, min/max labels, recommended range highlight
...
- [ ] On cancel: call `bookingService.cancelBooking()`, navigate to home

### 6.7 — Live Tracking Screen — Standard (`src/app/(customer)/tracking/[id].tsx`)

- [ ] Build tracking screen (full height):
...
- [ ] Extend standard tracking with transportation-specific features (same file, conditional rendering based on `booking.is_transportation`):
...
- [ ] Components needed:
  - [ ] Create `src/components/customer/BookingDetailSheet.tsx`:
...
- [ ] Real-time: subscribe to `user:{userId}` Supabase Realtime channel for new notifications
- [ ] Data fetching:
...
- [ ] Build profile screen (scrollable):
...
- [ ] Components needed:
  - [ ] Create `src/components/customer/ProfileMenuItem.tsx` — icon + label + chevron, pressable
  - [ ] Create `src/components/customer/EditProfileModal.tsx` — modal/screen with editable fields (name, phone, email, avatar), save → call `userService.updateProfile()`
...
- [ ] Create `UpdateRunnerProfileRequest.php`:
...
- [ ] Create `UploadDocumentRequest.php`:
...
- [ ] Create `UpdateLocationRequest.php`:
...
- [ ] Create `UpdateErrandStatusRequest.php`:
...
- [ ] Create `ToggleOnlineRequest.php`:
...
- [ ] Create `RunnerProfileController.php`:
  - `show()`:
...
- [ ] Create `RunnerDocumentController.php`:
...
- [ ] Create `RunnerOnlineController.php`:
...
- [ ] Create `RunnerLocationController.php`:
...
- [ ] Create `RunnerErrandController.php`:
  - `current()`:
...
- [ ] Create `RunnerEarningsController.php`:
  - `summary(Request)`:
...
- [ ] Create `RunnerErrandHistoryController.php`:
  - `index(Request)`:
...
- [ ] Create `RunnerPayoutController.php`:
...
- [ ] Add to `RunnerErrandController.php`:
...
- [ ] Update `app/Http/Resources/RunnerProfileResource.php` (extend from 3.4):
...
- [ ] Create `app/Http/Resources/RunnerDocumentResource.php`:
  - Fields: id, document_type, file_url, status, rejection_reason, created_at

- [ ] Create `app/Http/Resources/EarningsResource.php`:
...
- [ ] Create `app/Policies/RunnerProfilePolicy.php`:
  - `update($user, $profile)` — user owns this profile

- [ ] Create `app/Policies/RunnerErrandPolicy.php`:
...
- [ ] Define runner routes (middleware: `auth:sanctum`, `active`, `role:runner`):
  ```
...
- [ ] Create `app/Console/Commands/CleanupLocationsCommand.php`:
...
- [ ] Create `app/Events/RouteDeviationAlert.php`:
...
- [ ] Create `app/Events/RideDurationAlert.php`:
...
- [ ] Build online state (same file, conditional rendering based on `runnerStore.isOnline`):
...
- [ ] Background services when online:
...
- [ ] Components needed in `src/components/runner/`:
  - [ ] Create `OnlineToggle.tsx` — large toggle switch with status text and pulsing indicator
  - [ ] Create `StatCard.tsx` — icon + value + label in compact card
  - [ ] Create `NegotiateOfferCard.tsx` — booking summary card with offer amount, distance, countdown timer
  - [ ] Create `VerificationBanner.tsx` — status-dependent alert banner with action

### 8.3 — Incoming Errand Request — Fixed Price Modal

- [ ] Create `src/components/runner/IncomingRequestModal.tsx`:
...
- [ ] Background location: continue sending runner location every 5 seconds while errand active
- [ ] Components needed:
  - [ ] Create `src/components/runner/StatusActionButton.tsx` — dynamic button that changes label + action per status
  - [ ] Create `src/components/runner/ErrandDetailsCard.tsx` — collapsible card with task description, instructions, photos, value
  - [ ] Create `src/components/runner/NavigateButton.tsx` — floating button that opens external maps app

### 8.5 — Active Errand Screen — Transportation (`src/app/(runner)/errand/[id].tsx`)

- [ ] Extend standard errand screen for transportation (same file, conditional rendering):
...
- [ ] Create `src/components/runner/PhotoProofModal.tsx`:
  - `FloatingModal`
...
- [ ] Create `src/components/runner/CompletionModal.tsx`:
...
- [ ] Same chat interface as customer (6.9), but from runner's perspective:
...
- [ ] Create or navigate to a detail view (can be a new screen or `BottomSheet`):
...
- [ ] Data fetching: all data from `bookingService.getBooking(id)` with relationships
...
- [ ] Components needed:
  - [ ] Create `src/components/runner/PerformanceMetric.tsx` — circular progress ring with percentage, label underneath

### 8.13 — Documents & Verification Screen

- [ ] Create standalone screen (navigate from profile):
...
- [ ] Components needed:
  - [ ] Create `src/components/runner/DocumentUploadCard.tsx` — document type label, current status badge, thumbnail/placeholder, upload/replace button, rejection reason display
...
- [ ] Reuse same notification screen structure as customer (6.12):
...
- [ ] Create `src/components/runner/RateCustomerModal.tsx`:
...
- [ ] Implement `src/hooks/useSupabaseRealtime.ts`:
...
- [ ] Implement `src/hooks/useRunnerTracking.ts`:
  - Subscribe to Supabase Realtime channel: `booking:{bookingId}`
...
- [ ] Implement runner-side location broadcasting:
...
- [ ] Implement booking status subscription in tracking screen:
  - Subscribe to Supabase Realtime channel: `booking:{bookingId}`
...
- [ ] Implement runner-side incoming request subscription:
...
- [ ] Backend: when message is created via `POST /chat/{bookingId}/messages`:
...
- [ ] Implement notification subscription in app root layout:
...
### 10.1 — Stripe Integration (Backend)
...
- [ ] Create `app/Services/PaymentService.php`:
  ...
  - `refundPayment($paymentId, $amount)`:
    ...
    - Update payment record (refund_amount, refunded_at)
    - Credit customer wallet if wallet payment
  - `createCustomer($userId, $email)` — create Stripe Customer for tokenized payments
  - `attachPaymentMethod($customerId, $paymentMethodToken)` — attach card to Stripe Customer
  - `detachPaymentMethod($paymentMethodId)` — remove card from Stripe

- [ ] Create `app/Http/Controllers/Payment/StripeWebhookController.php`:
  - Handle `payment_intent.succeeded` → update payment status to completed, update booking
  - Handle `payment_intent.payment_failed` → update payment status to failed, notify customer
  - Handle `charge.refunded` → update payment refund status
  - Verify Stripe webhook signature for security

### 10.2 — GCash / PayPal Integration (Backend)

- [ ] Add GCash payment processing to `PaymentService.php`:
  - `createGCashPayment($amount, $bookingId)` — redirect flow to GCash payment page
  - `handleGCashCallback($data)` — process GCash payment confirmation
  - Use GCash API or payment gateway that supports GCash (e.g., PayMongo, Dragonpay)

- [ ] Add PayPal payment processing to `PaymentService.php`:
  - `createPayPalOrder($amount, $currency)` — create PayPal Order
  - `capturePayPalOrder($orderId)` — capture after customer approves
  - Handle PayPal webhook for payment confirmation
...
- [ ] Create `app/Http/Controllers/Payment/PaymentMethodController.php`:
  - `index()` — list user's payment methods (masked card info)
  - `store(Request)` — add new payment method (card via Stripe token, GCash, PayPal)
  - `destroy($id)` — remove payment method (verify ownership)
  - `setDefault($id)` — set as default, unset other defaults

- [ ] Create `app/Http/Controllers/Payment/WalletController.php`:
...
- [ ] Create `app/Http/Controllers/Payment/PaymentHistoryController.php`:
...
- [ ] Define payment routes (middleware: `auth:sanctum`, `active`):
  ```
...
- [ ] Set up Stripe SDK on frontend:
  - Initialize `@stripe/stripe-react-native` `StripeProvider` in root layout with publishable key
  - Wrap payment screens with Stripe context

- [ ] Implement card payment flow in `PaymentMethodSelector`:
  - "Add Card" → open Stripe `CardField` or `PaymentSheet`
  - Tokenize card details client-side (never send raw card data to your server)
  - Send token to backend → backend calls `attachPaymentMethod()`

- [ ] Implement GCash payment flow:
  - On select GCash: redirect to GCash payment page (WebView or external browser)
  - Handle redirect callback back to app (deep link)

- [ ] Implement PayPal payment flow:
  - On select PayPal: open PayPal checkout (WebView or PayPal SDK)
...
- [ ] Implement `src/hooks/useNotifications.ts`:
...
- [ ] Define notification templates in `app/Notifications/` (or as constants):
...
- [ ] Create `app/Services/SOSService.php`:
...
- [ ] Create `app/Http/Controllers/Customer/SOSController.php`:
  - `trigger($bookingId)`:
...
- [ ] Create `app/Http/Controllers/Customer/TripShareController.php`:
  - `share($bookingId)`:
...
- [ ] Create `app/Http/Controllers/PublicTripController.php` (no auth):
  - `show($token)`:
...
- [ ] Implement `TripShareSheet` component behavior:
...
- [ ] Implement in `LocationService.php`:
...
- [ ] `SOSButton` component (in tracking screen for transportation):
...
- [ ] Create `tests/Feature/Auth/SocialLoginTest.php`:
...
- [ ] Create `tests/Feature/Booking/TrackingTest.php`:
  - Test track endpoint returns runner location
  - Test only booking participants can track
- [ ] Create `tests/Feature/Runner/OnlineToggleTest.php`:
...
- [ ] Create `tests/Feature/Payment/StripeWebhookTest.php`:
  - Test payment_intent.succeeded handling
  - Test payment_intent.payment_failed handling
...
- [ ] Set up testing environment:
  - Install `@testing-library/react-native`, `jest`, `jest-expo`
  - Configure `jest.config.js` for Expo
  - Create mock factories for common types (User, Booking, RunnerProfile, etc.) in `src/__mocks__/`
  - Create API mock helpers (mock Axios responses) in `src/__mocks__/services/`

- [ ] Create component tests (`src/components/ui/__tests__/`):
  - `Button.test.tsx` — render variants, loading state, disabled, press handler
  - `Input.test.tsx` — render, validation error display, keyboard type
  - `OTPInput.test.tsx` — digit entry, auto-focus, backspace, completion callback
  - `RatingStars.test.tsx` — interactive selection, display mode
  - `PriceBreakdown.test.tsx` — renders items + total correctly
  - `StatusTimeline.test.tsx` — renders steps, highlights current
  - `Badge.test.tsx` — renders count/label, variants
  - `BottomSheet.test.tsx` — open/close, snap points
...
- [ ] Create store tests (`src/stores/__tests__/`):
  - `authStore.test.ts` — setUser, setToken, logout, persistence
  - `bookingStore.test.ts` — draft management, step progression
  - `runnerStore.test.ts` — online toggle, incoming request

### 13.3 — End-to-End Testing

- [ ] Set up Detox or Maestro for E2E testing
- [ ] Create E2E test flows:
  - `auth-flow.e2e.ts` — register → verify OTP → select role → land on home
  - `booking-flow.e2e.ts` — create booking → searching → matching → tracking → completion → review
  - `runner-flow.e2e.ts` — go online → receive request → accept → complete errand → earnings update

### 13.4 — Security Audit Checklist

- [ ] Verify all API endpoints require proper authentication (no open endpoints except auth + public trip)
- [ ] Verify role-based access control on all routes (customer can't access runner routes, vice versa)
- [ ] Verify Supabase RLS policies prevent cross-user data access
- [ ] Verify all user inputs are validated via Form Requests (no raw request data used)
- [ ] Verify parameterized queries only (no raw SQL concatenation)
- [ ] Verify passwords are Bcrypt-hashed (cost factor 12)
- [ ] Verify bank account numbers are AES-256-GCM encrypted
- [ ] Verify Stripe webhook signature validation
- [ ] Verify file upload MIME type validation (not just extension)
- [ ] Verify rate limiting on all auth routes + OTP
- [ ] Verify API security headers present on all responses
- [ ] Verify Sanctum tokens are hashed (SHA-256) in database
- [ ] Verify no sensitive data in API responses (password_hash, gateway_tokens, etc.)
- [ ] Verify image uploads are processed/resized (prevent malicious large files)
- [ ] Verify location data deleted after 24 hours
- [ ] Verify chat messages deleted 30 days post-booking-completion

### 13.5 — Performance Optimization

- [ ] Frontend optimizations:
  - Implement `FlatList` performance: `getItemLayout`, `maxToRenderPerBatch`, `windowSize`, `removeClippedSubviews`
  - Image optimization: cache with `expo-image`, lazy load, proper sizing
  - Mapbox performance: minimal re-renders, memoize markers, limit polyline point density
  - Bundle size: audit with `npx expo-optimize`, tree-shake unused imports
...
- [ ] Backend optimizations:
  - Database indexing: verify all indexes from Phase 2 are created
  - Eager loading relationships in controllers (prevent N+1 queries)
  - Cache errand types + system config (Redis/file cache, bust on update)
  - Queue heavy operations: email sending, push notifications, image processing
  - Rate limit location updates to 1 per 5 seconds per runner
  - Paginate all list endpoints

### 13.6 — Environment & Deployment Setup

- [ ] Backend deployment (Laravel Forge):
  - Set up server on Laravel Forge (DigitalOcean, AWS, etc.)
  - Configure Nginx + PHP-FPM
  - Set up SSL certificate (Let's Encrypt)
  - Configure environment variables on server
  - Set up queue worker (Supervisor) for background jobs
  - Set up cron for scheduled commands (location cleanup, duration checks)
  - Configure database connection to Supabase PostgreSQL
  - Set up Redis for caching + queue driver

- [ ] Frontend deployment (Expo):
  - Configure `eas.json` for EAS Build:
    - `development` profile: dev client build
    - `preview` profile: internal distribution (APK/IPA for testing)
    - `production` profile: store submission build
  - Set up EAS Update for over-the-air updates
  - Configure app signing: Android keystore, iOS certificates + provisioning profiles
  - Set environment variables per profile in `eas.json`

- [ ] CI/CD Pipeline:
  - Set up GitHub Actions workflow:
    - On PR: run backend tests (PHPUnit), run frontend tests (Jest), lint check
    - On merge to `main`: deploy backend to Laravel Forge, trigger EAS Build for preview
    - On release tag: trigger production EAS Build + submit to stores
