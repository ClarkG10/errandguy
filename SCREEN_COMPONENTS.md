...
- **Hooks** (`src/hooks/`): `useAuth`, `useBooking`, `useBookingStatus`, `useChat`, `useCountdown`, `useEta`, `useIncomingRequest`, `useNotifications`, `useQuery`, `useRunnerTracking`, `useForegroundInterval`, `useReducedMotion`, `useImagePicker`, `useDebounce`, `useKeyboard`, `useBackGuard`.
- **Services** (`src/services/`): `api`, `auth`, `booking`, `chat`, `config`, `geocoding`, `notification`, `payment`, `route`, `runner`, `user`, `cache`, `preload`.
